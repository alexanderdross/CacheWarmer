...
// Cache auspacken
$kpi_row         = $cache_data['kpi_row'];
$install_kpi     = $cache_data['install_kpi'];
$tier_counts     = $cache_data['tier_counts'];
$platform_counts = $cache_data['platform_counts'];
$timeline_raw    = $cache_data['timeline_raw'];

$total_licenses    = (int) ( $kpi_row->total_licenses ?? 0 );
$active_licenses   = (int) ( $kpi_row->active_licenses ?? 0 );
$grace_licenses    = (int) ( $kpi_row->grace_licenses ?? 0 );
$expiring_soon     = (int) ( $kpi_row->expiring_soon ?? 0 );
$active_installs   = (int) ( $install_kpi->active_installs ?? 0 );
$today_activations = (int) ( $install_kpi->today_activations ?? 0 );
...
for ( $i = 29; $i >= 0; $i-- ) {
    ...
    $timeline_data[]   = (int) ( $timeline_raw[ $date ]->cnt ?? 0 );
}
