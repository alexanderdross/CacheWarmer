...
vi.mock("@/lib/services/facebook-warmer", () => ({
  warmFacebook: vi.fn().mockResolvedValue([]),
}));
...
vi.mock("@/lib/services/indexnow", () => ({
  submitIndexNow: vi.fn().mockResolvedValue({ status: "success", urlCount: 0 }),
}));
...
vi.mock("@/lib/services/bing-indexer", () => ({
  submitToBing: vi.fn().mockResolvedValue({ status: "success", urlCount: 0 }),
}));
...
describe("POST /api/warm", () => {
  ...
  it("should filter out invalid targets", async () => {
    ...
    const request = new NextRequest("http://localhost:3000/api/warm", {
      ...
      body: JSON.stringify({
        sitemapUrl: "https://example.com/sitemap.xml",
        targets: ["cdn", "invalid-target", "facebook"],
      }),
    });
